...
#Version: 2.0                                       #
#Last mod: 01/04/23 10:00                           #
#####################################################

#Comandi Commentati da eseguire solo al primo avvio
...
ui <- fluidPage(theme = shinytheme("cyborg"),
    tabPanel(title = "CHNet software engine by nemesys co.",  value = "panel",
         # Panel per titolo
         titlePanel("CHNet software engine by nemesys co."),
         ...
         sidebarLayout(
           ...
           sidebarPanel(
             ...
             # Selezione file
             fileInput("file", label = h4("Choose data file")),
             
             # Divisore
             tags$hr(),
             
             # Visualizzazione descrittore file 1
             fluidRow(column(12, verbatimTextOutput("val1"))),
             ...
             # Selezione file
             fileInput("label", label = h4("Choose label file")),
             
             # Divisore
             tags$hr(),
             
             # Visualizzazione descrittore file 1
             fluidRow(column(12, verbatimTextOutput("val2"))),
             
             # Divisore
             tags$hr(),
             
             # Numero di righe da mostrare
             radioButtons("varBool", label = h4( "Subsampling"),
                          choices = c("True" = TRUE,
                                      "False" = FALSE),
                          selected = FALSE),
                          ...
                          fluidRow(column(12, verbatimTextOutput("boolChoose"))),
             ...
             fluidRow(column(12,
                      sliderInput("slider", label = h4("Number of observations to view (R param.):"), min = 1, 
                                  max = 100, value = 50))),
                         
                      # Divisore
                      tags$hr(),
               
                      fluidRow(column(12, verbatimTextOutput("num1"))),
             
             #Input per valore numerico lambda (2- +inf.)
             numericInput(inputId = "obs",
                          label =  h4("Lambda value:"),
                          value = 0.1),
             ...
             actionButton("GO", "Procedi col caricamento", class = "btn-primary")
...
server <- function(input, output, session) {
  ...
  #Plotta descrittore file
  output$val1 <- renderPrint({str(input$file)})
  
  #Plotta descrittore label
  output$val2 <- renderPrint({str(input$label)})
  
  #Plotta anteprima radiobutton
  output$boolChoose <- renderPrint({ input$varBool })
  
  #Plotta anteprima slider
  output$num1 <- renderPrint({ input$slider })
  
  #Plotta anteprima di Lambda
  output$num2 <- renderPrint({ input$obs })
  
  observeEvent(input$GO, {
    
    #Primo file con plot header
    df1 <- eventReactive(input$file, {
      dfdata1 <- data.frame(read_xlsx(input$file$datapath, col_types = "numeric"))
    })
    output$Data1 <- renderTable( head( df1()))
    
    #Secondo file con plot header
    df2 <- eventReactive(input$label, {
      dfdata2 <- data.frame(read_xlsx(input$label$datapath, col_types = "numeric"))
    })
    ...
    output$plot <- renderPlot({
      ...
      heatmap_plot <- ggplot(adj_matrix_long, aes(x = Var1, y = Var2, fill = value)) +
        geom_tile() +
    ...
    #output$plot2 <- renderPlot({
     ...
     # network_graph <- result$Diff.net.connected
